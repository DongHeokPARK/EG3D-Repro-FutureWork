...
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=128, show_default=True)     # 수정 전 : 512
...
@click.option('--kimg',         help='Total training duration', metavar='KIMG',                 type=click.IntRange(min=1), default=25, show_default=True)  # 수정 전 : default 25000
...
@click.option('--neural_rendering_resolution_initial', help='Resolution to render at', metavar='INT',  type=click.IntRange(min=1), default=64, required=False)   # 수정 전 : default=64
@click.option('--neural_rendering_resolution_final', help='Final resolution to render at, if blending', metavar='INT',  type=click.IntRange(min=1), required=False, default=64)   # 수정 전 : default=None
...
@click.option('--sr_num_fp16_res',    help='Number of fp16 layers in superresolution', metavar='INT', type=click.IntRange(min=0), default=2, required=False, show_default=True)     # 수정 전 : default=4
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=128, w_dim=128, mapping_kwargs=dnnlib.EasyDict())   # 수정 전 : z_dim=512, w_dim=512
    ...
    if opts.cfg == 'ffhq':
        ...
        rendering_options.update({
            'depth_resolution': 32,                 # 수정 전 : 64
            'depth_resolution_importance': -1,      # 수정 전 : 64
            'ray_start': 0.1,
            'ray_end': 2.6,
            'box_warp': 1.6,
            'white_back': True,
